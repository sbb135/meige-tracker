...
const MeigeTracker = () => {
  ...
  const [newDoctor, setNewDoctor] = useState({ nome: '', especialidade: '', local: '', notas: '', proximaConsulta: '', color: 'sky' });
      ...
      try {
        ...
        if (entriesData) {
          ...
          entriesData.forEach(e => {
            entriesObj[e.entry_date] = {
              ...
              wakeCrying: e.wake_crying || false,
              wakeStabilizeTime: e.wake_stabilize_time || 0,
              wakeEyes: e.wake_eyes || 0,
              wakeFace: e.wake_face || 0,
              wakeNeck: e.wake_neck || 0,
              wakeEyesFreq: e.wake_eyes_freq || 0,
              wakeSpeech: e.wake_speech || 0,
              wakeEating: e.wake_eating || 0,
              morningEyes: e.morning_eyes || 0,
              morningFace: e.morning_face || 0,
              morningNeck: e.morning_neck || 0,
              morningEyesFreq: e.morning_eyes_freq || 0,
              morningSpeech: e.morning_speech || 0,
              morningEating: e.morning_eating || 0,
              afternoonEyes: e.afternoon_eyes || 0,
              afternoonFace: e.afternoon_face || 0,
              afternoonNeck: e.afternoon_neck || 0,
              afternoonEyesFreq: e.afternoon_eyes_freq || 0,
              afternoonSpeech: e.afternoon_speech || 0,
              afternoonEating: e.afternoon_eating || 0,
              eveningEyes: e.evening_eyes || 0,
              eveningFace: e.evening_face || 0,
              eveningNeck: e.evening_neck || 0,
              eveningEyesFreq: e.evening_eyes_freq || 0,
              eveningSpeech: e.evening_speech || 0,
              eveningEating: e.evening_eating || 0,
              triggers: e.triggers || {},
              sideEffects: e.side_effects || {},
              botoxEffect: e.botox_effect || '',
              dailyLimitation: e.daily_limitation || 0,
              hadGoodPeriod: e.had_good_period || false,
              goodPeriodWhen: e.good_period_when || '',
              goodPeriodDuration: e.good_period_duration || '',
              normalTasks: e.normal_tasks || '',
              cryingEpisodes: e.crying_episodes || 0,
              mood: e.mood || '',
        ...
        if (botoxData) {
          setBotoxRecords(botoxData.map(b => ({
            id: b.id,
            date: b.date || b.injection_date, // Handle both column names
  ...
  const saveEntry = async () => {
    ...
    try {
      ...
      await supabase.from('daily_entries').upsert({
        ...
        sleep_interruptions: dayEntry.sleepInterruptions,
        felt_rested: dayEntry.feltRested,
        emotional_dysregulation: dayEntry.emotionalDysregulation,
        wake_crying: dayEntry.wakeCrying,
        wake_stabilize_time: dayEntry.wakeStabilizeTime,
        wake_eyes: dayEntry.wakeEyes,
        wake_face: dayEntry.wakeFace,
        wake_neck: dayEntry.wakeNeck,
        wake_eyes_freq: dayEntry.wakeEyesFreq,
        wake_speech: dayEntry.wakeSpeech,
        wake_eating: dayEntry.wakeEating,
        morning_eyes: dayEntry.morningEyes,
        morning_face: dayEntry.morningFace,
        morning_neck: dayEntry.morningNeck,
        morning_eyes_freq: dayEntry.morningEyesFreq,
        ...
        afternoon_neck: dayEntry.afternoonNeck,
        afternoon_eyes_freq: dayEntry.afternoonEyesFreq,
        ...
        evening_neck: dayEntry.eveningNeck,
        evening_eyes_freq: dayEntry.eveningEyesFreq,
  ...
  const renderConsultasCalendar = () => {
    ...
    const getAppointmentsForDate = (dateStr) => {
      const appointments = [];
      medicos.forEach(m => {
        if (m.proximaConsulta === dateStr) appointments.push({ tipo: m.especialidade, nome: m.nome, isUpcoming: true, color: m.color, hora: m.proximaHora });
        if (m.ultimaConsulta === dateStr) appointments.push({ tipo: m.especialidade, nome: m.nome, isUpcoming: false, color: m.color });
      });
      consultas.forEach(c => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // Use custom color if available, otherwise fallback to specialty-based color
      const getCustomColor = (appt) => {
        if (appt.color) {
          const colorMap = {
            purple: 'bg-purple-700/60', indigo: 'bg-indigo-700/60', teal: 'bg-teal-700/60',
            green: 'bg-green-700/60', sky: 'bg-sky-700/60', amber: 'bg-amber-700/60',
            rose: 'bg-rose-700/60', orange: 'bg-orange-700/60'
          };
          return colorMap[appt.color] || 'bg-sky-700/60';
        }
        return getCellBgColor(appt.tipo);
      };
      const getCustomBadgeColor = (appt) => {
        if (appt.color) {
          const colorMap = {
          ...
          return colorMap[appt.color] || 'bg-sky-500';
        }
        return getBadgeColor(appt.tipo);
      };
      const bgColor = hasAppointment ? getCustomColor(appointments[0]) : 'bg-slate-700';
      ...
      days.push(
        <div
          ...
          title={appointments.map(a => `${a.tipo}: ${a.nome}${a.hora ? ' às ' + a.hora : ''}`).join('\n')}
        >
          <span className={`text-base ${isToday ? 'text-sky-300 font-bold' : hasAppointment ? 'text-white font-bold' : 'text-slate-300'}`}>
            {day}
          </span>
          {hasAppointment && (
            <span className={`mt-0.5 text-[8px] px-1.5 py-0.5 rounded-full font-semibold ${getCustomBadgeColor(appointments[0])} text-white`}>
              {appointments[0].tipo}
            </span>
          )}
        </div>
  ...
  const renderConsultas = () => {
    ...
    const updateDoctorAppointment = (doctorId, newDate, newTime) => {
      setMedicos(medicos.map(m =>
        m.id === doctorId ? { ...m, proximaConsulta: newDate, proximaHora: newTime } : m
      ));
                  ...
                  <div className="mt-3 p-3 bg-slate-600 rounded">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      <input
                        ...
                        className="p-2 rounded bg-slate-700 text-slate-100 text-sm border border-slate-500"
                        id={`edit-date-${m.id}`}
                      />
                      <input
                        type="time"
                        defaultValue={m.proximaHora || ''}
                        className="p-2 rounded bg-slate-700 text-slate-100 text-sm border border-slate-500"
                        id={`edit-time-${m.id}`}
                        placeholder="Hora"
                      />
                  ...
                  <>
                    {m.proximaConsulta && (
                      <p className="text-sm text-sky-400">
                        Próxima: {formatDatePT(m.proximaConsulta)}{m.proximaHora ? ` às ${m.proximaHora}` : ''}
                      </p>
                    )}
          ...
          {!showAddDoctor ? (
            ...
            <div className="mt-4 p-4 bg-slate-700 rounded-lg border border-slate-600">
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                ...
                <input
                  type="text"
                  placeholder="Especialidade (ex: Dentista)"
              ...
              {/* Color Picker */}
              <div className="mb-3">
                <label className="block text-xs text-slate-400 mb-2">Cor do badge</label>
                <div className="flex gap-2 flex-wrap">
                  {[
                    { name: 'purple', label: 'Roxo', bg: 'bg-purple-500' },
                    { name: 'indigo', label: 'Índigo', bg: 'bg-indigo-500' },
                    { name: 'teal', label: 'Verde-azul', bg: 'bg-teal-500' },
                    { name: 'green', label: 'Verde', bg: 'bg-green-500' },
                    { name: 'sky', label: 'Azul', bg: 'bg-sky-500' },
                    ...
                    <button
                      key={c.name}
                      onClick={() => setNewDoctor({ ...newDoctor, color: c.name })}
                      className={`w-8 h-8 rounded-full ${c.bg} ${newDoctor.color === c.name ? 'ring-2 ring-white ring-offset-2 ring-offset-slate-700' : ''}`}
                      title={c.label}
                    />
                  ))}
                </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    if (newDoctor.nome && newDoctor.especialidade && newDoctor.local) {
                      setMedicos([...medicos, { ...newDoctor, id: Date.now() }]);
                      setNewDoctor({ nome: '', especialidade: '', local: '', notas: '', proximaConsulta: '', color: 'sky' });
